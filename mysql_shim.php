...
namespace {
    if ( !function_exists( '\mysql_connect' ) ) {
        ...
        $hostname = null, $username = null, $password = null, $new = false, $flags = 0
        ) {
            if ( $new !== false ) {
            ...
            if ( $flags === 0 ) {
                ...
                $conn->hash = $hash;
                \Dshafik\MySQL::$connections[$hash] = [
                    'refcount' => 1,
                    'conn' => $conn ];
                return $conn;
            }
            ...
            try {
                ...
                mysqli_real_connect(
                        $conn, $hostname, $username, $password, '', null, '', $flags
                );
        ...
        $hostname = null, $username = null, $password = null, $flags = 0
        ) {
            $hostname = 'p:' . $hostname;
        ...
        function mysql_select_db( $databaseName, \mysqli $link = null ) {
            ...
            return mysqli_query(
                            $link, "USE `" . mysqli_real_escape_string( $link, $databaseName ) . "`"
                    ) !== false;
        }
        ...
        function mysql_list_tables( $databaseName, \mysqli $link = null ) {
            $link = \Dshafik\MySQL::getConnection( $link );
            $query = sprintf(
                    "SHOW TABLES FROM `%s`", mysql_real_escape_string( $databaseName, $link )
            );
            return mysql_query( $query, $link );
        }
        ...
        function mysql_list_fields( $databaseName, $tableName, \mysqli $link = null ) {
            $link = \Dshafik\MySQL::getConnection( $link );
            $query = sprintf(
                    "SHOW COLUMNS FROM `%s`.`%s`", mysqli_real_escape_string( $link, $databaseName ), mysqli_real_escape_string( $link, $tableName )
            );
        ...
        function mysql_result( $result, $row, $field = 0 ) {
            ...
            if ( !mysqli_data_seek( $result, $row ) ) {
                trigger_error(
                        sprintf(
                                "mysql_result(): Unable to jump to row %d on MySQL result index %s", $row, spl_object_hash( $result )
                        ), E_USER_WARNING
                );
                // @codeCoverageIgnoreStart
                return false;
            ...
            trigger_error(
                    sprintf(
                            "%s(): %s not found in MySQL result index %s", __FUNCTION__, $field, spl_object_hash( $result )
                    ), E_USER_WARNING
            );
            // @codeCoverageIgnoreStart
            return false;
        ...
        function mysql_escape_string( $unescapedString ) {
            if ( \Dshafik\MySQL::$last_connection === null ) {
                trigger_error(
                        sprintf(
                                "%s() is insecure; use mysql_real_escape_string() instead!", __FUNCTION__
                        ), E_USER_NOTICE
                );
    ...
    class MySQL {
        ...
        public static function mysqlFieldInfo( \mysqli_result $result, $field, $what ) {
            try {
                ...
                trigger_error(
                        sprintf(
                                "mysql_field_%s(): Field %d is invalid for MySQL result index %s", $what, $field, spl_object_hash( $result )
                        ), E_USER_WARNING
                );
                // @codeCoverageIgnoreStart
                // PHPUnit turns the warning into an exception, so this never runs
        ...
        public static function checkValidResult( $result, $function ) {
            if ( !($result instanceof \mysqli_result) ) {
                if ( $function != "mysql_fetch_object" ) {
                    trigger_error(
                            $function . "() expects parameter 1 to be resource, " . strtolower( gettype( $result ) ) . " given", E_USER_WARNING
                    );
                }
                if ( $function == "mysql_fetch_object" ) {
                    trigger_error(
                            $function . "(): supplied argument is not a valid MySQL result resource", E_USER_WARNING
                    );
                }
                return false;
            }
