...
class JobFilter implements \IWPML_Backend_Action, \IWPML_Frontend_Action {
	...
	public function reorderFields( array $fields, \stdClass $job ): array {
		...
		$mailSorter = function ( array $rowA, array $rowB ): int {
			...
			if ( $a['id'] === $b['id'] ) {
				$isSubjectA = $a['is_sub_field_is_subject'];
				$isSubjectB = $b['is_sub_field_is_subject'];

				if ( $isSubjectA && ! $isSubjectB ) {
					return -1;
				} elseif ( ! $isSubjectA && $isSubjectB ) {
					return 1;
				}

				return $rowA['__sort_helper']['original_index'] - $rowB['__sort_helper']['original_index'];
			}
	...
	private function prepareFieldsToSort( array $fields ): array {
		...
		return array_map(
			function ( array $field, int $index ) use ( $callbackPrepareGroup ): array {
