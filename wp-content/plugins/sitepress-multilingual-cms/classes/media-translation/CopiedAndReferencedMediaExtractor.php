...
class CopiedAndReferencedMediaExtractor {
	...
	public function extract( $post, $get_attachment_ids_from_urls = true ) {
		...
		if ( $can_batch && $get_attachment_ids_from_urls && $not_classified_media_in_tags ) {
			...
			foreach ( $not_classified_media_in_tags as $media_in_tag ) {
				...
				if ( '' !== $src ) {
					$not_classified_srcs[ $src ] = true;
				}
			}
			if ( $not_classified_srcs ) {
				$not_classified_ids = Attachment::attachmentUrlsToPostIds( array_keys( $not_classified_srcs ) );
			}
		}
