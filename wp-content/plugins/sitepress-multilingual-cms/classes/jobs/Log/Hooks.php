...
class Hooks implements \IWPML_Backend_Action, \IWPML_DIC_Action {
	...
	public function add_hooks() {
		...
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_toggle_feature', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxToggle' ] );
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_clear', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxClear' ] );
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_save_caps', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxSaveCaps' ] );
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_download', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxDownload' ] );
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_download_all', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxDownloadAll' ] );
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_download_last_100', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxDownloadLast100' ] );
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_get_request_detail', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxGetRequestDetail' ] );
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_search_posts', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxSearchPosts' ] );
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_entity_timeline', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxEntityTimeline' ] );
		\WPML\Request\Adapter\Ajax::register( 'wpml_tm_job_log_download_entity_timeline', \WPML\Request\Policy\Policy::capability( 'manage_options', \WPML\Request\Policy\Authenticity::actionNonce( 'wpml_tm_job_log', 'nonce' ) ), [ $this, 'handleAjaxDownloadEntityTimeline' ] );
	}
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] === self::SUBMENU_HANDLE ) {
			...
			wp_localize_script(
				...
				array(
					...
					/* translators: Message shown when the search on the translation jobs screen could not be carried out. */
	...
	public function handleAjaxSaveCaps() {
		...
		if ( $maxMb < FsJobLogStorage::MIN_CONFIGURABLE_MB || $maxMb > FsJobLogStorage::MAX_CONFIGURABLE_MB ) {
			wp_send_json_error(
                sprintf(
				/* translators: Error message in the dialog that sets the limits of the job log. %1$d: the smallest size that may be entered, %2$d: the largest, both in megabytes. */
		...
		if ( $maxFiles < FsJobLogStorage::MIN_CONFIGURABLE_FILES || $maxFiles > FsJobLogStorage::MAX_CONFIGURABLE_FILES ) {
			wp_send_json_error(
                sprintf(
				/* translators: Error message in the dialog that sets the limits of the job log. %1$d: the smallest number of requests that may be kept, %2$d: the largest. */
		...
		global $sitepress;
		if ( ! $sitepress ) {
			/* translators: Message shown when the limits of the job log cannot be read or saved. */
