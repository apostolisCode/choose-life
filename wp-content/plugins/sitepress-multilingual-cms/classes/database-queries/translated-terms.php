...
class TranslatedTerms {
	...
	public static function getIdsForLangs( $langs ) {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT wpt.term_id, wptt.taxonomy
				FROM {$wpdb->terms} wpt
				INNER JOIN {$wpdb->term_taxonomy} wptt ON wptt.term_id = wpt.term_id
				...
				WHERE iclt.language_code IN (" . implode( ', ', array_fill( 0, count( $langs ), '%s' ) ) . ')',
				$langs
			)
		);
	}
