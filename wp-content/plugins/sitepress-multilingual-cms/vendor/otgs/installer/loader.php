	...
	if ( ! function_exists( 'WP_Installer_Setup' ) ) {
		function WP_Installer_Setup( $wp_installer_instance, $args = [] ) {
			if ( isset( $args['site_key_nags'][0]['repository_id'] ) ) {
				...
				require_once __DIR__ . '/includes/utilities/OutboundLink.php';
				...
				if ( in_array( $repository_id, [ 'wpml', 'toolset' ] )
					...
					$showFrontendBanner = function () use ( $repository_id ) {
						$removeFrontendBannerLink = \OTGS\Installer\OutboundLink::to(
							'https://wpml.org/troubleshooting/site-keys/',
							[ 'medium' => 'notice', 'campaign' => 'account', 'content' => 'footer-notice' ]
						);
						$wpmlHomeLink = \OTGS\Installer\OutboundLink::to(
							'https://wpml.org/',
							[ 'medium' => 'notice', 'campaign' => 'account', 'content' => 'footer-notice' ]
						);
						$wpmlText = sprintf(
							__( 'This site is registered on %s as a development site. Switch to a production site key to %s.', 'installer' ),
							'<a href="' . $wpmlHomeLink . '">wpml.org</a>', '<a href="' . $removeFrontendBannerLink . '">remove this banner</a>'
						);
