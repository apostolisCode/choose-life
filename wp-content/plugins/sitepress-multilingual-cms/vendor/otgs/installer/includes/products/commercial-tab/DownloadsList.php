...
class DownloadsList {
	public static function getDownloadRow( $download_id, $download, $site_key, $repository_id ) {
		...
		$leadDescription = '';
		if ( isset( $download['lead_description'] ) && is_array( $download['lead_description'] ) ) {
			$localizedLeadDescription = Language::getValue( $download['lead_description'] );
			if ( $localizedLeadDescription !== '' ) {
				$leadDescription = '<p id="'.$download_id.'_lead-description">'.$localizedLeadDescription.'</p>';
			}
		}
