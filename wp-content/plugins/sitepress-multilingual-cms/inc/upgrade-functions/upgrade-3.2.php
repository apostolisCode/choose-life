...
$wpdb->query( "ALTER TABLE {$wpdb->prefix}icl_translate MODIFY COLUMN field_type VARCHAR( 160 ) NOT NULL" );

$batch_id_exists = $wpdb->get_var(
	$wpdb->prepare(
...
if ( ! $batch_id_exists || ! (int) $batch_id_exists ) {
	$wpdb->query( "ALTER TABLE `{$wpdb->prefix}icl_translation_status` ADD batch_id int DEFAULT 0 NOT NULL;" );
}
...
if ( ! $batch_id_exists || ! (int) $batch_id_exists ) {
	$wpdb->query( "ALTER TABLE `{$wpdb->prefix}icl_string_translations` ADD batch_id int DEFAULT -1 NOT NULL;" );
...
if ( ! $batch_id_exists || ! (int) $batch_id_exists ) {
	$wpdb->query( "ALTER TABLE `{$wpdb->prefix}icl_string_translations` ADD translation_service varchar(16) DEFAULT '' NOT NULL;" );
}
...
if ( ! in_array( 'string_package_id', $icl_strings_columns ) ) {
	$wpdb->query(
        ...
        ADD `title` TEXT NULL AFTER type,
