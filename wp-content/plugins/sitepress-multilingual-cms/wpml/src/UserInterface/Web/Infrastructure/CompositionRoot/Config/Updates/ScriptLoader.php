...
class ScriptLoader {
  ...
  public function loadScript( $idsOfUpdatesToPerform, $endpoint ) {
    ...
    $this->page->deferScriptLoading(
      function () use ( $idsOfUpdatesToPerform, $endpoint ) {
        $this->enqueueScript( $idsOfUpdatesToPerform, $endpoint );
      }
    );
  }


  private function enqueueScript( $idsOfUpdatesToPerform, $endpoint ) {
