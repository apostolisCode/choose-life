...
class PostsFilter implements PostsFilterInterface {
  ...
  public function filter( array $posts, array $searchCriteria ): array {
    return apply_filters( 'wpml_tm_dashboard_posts', $this->addBlockedProps( $posts ), $searchCriteria );
  }


  private function addBlockedProps( array $posts ): array {
    $postIds = array_map(
      function( $post ) {
        return $post['id'];
      },
    ...
    foreach ( $posts as &$post ) {
      $isBlocked = isset( $blockedPosts[ $post['id'] ] );

      $post['isBlocked']     = $isBlocked;
      $post['blockedReason'] = $isBlocked && $blockedPosts[ $post['id'] ] !== ''
        ? $blockedPosts[ $post['id'] ]
        : null;
    }
