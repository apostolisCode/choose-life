...
class Config implements ConfigInterface {
  ...
  public function loadAdminStyles() {
    $config = $this->parser->parseStyles();
    foreach ( $config->styles() as $style ) {
      $style->onlyRegister()
        ? $this->page->registerStyle( $style )
  ...
  public function loadAdminScripts() {
    ...
    $this->loadScripts( $config->scripts(), true );
  }
  ...
  private function loadScripts( $scripts, $checkUsedOnAdmin = false ) {
    foreach ( $scripts as $script ) {
      ...
      if ( $checkUsedOnAdmin && ! $script->usedOnAdmin() ) {
        continue;
      }
  ...
  private function provideEndpoints( $endpoints, $scriptForData = null ) {
    ...
    $hasOwnEndpoints = count( $endpoints ) > 0;
