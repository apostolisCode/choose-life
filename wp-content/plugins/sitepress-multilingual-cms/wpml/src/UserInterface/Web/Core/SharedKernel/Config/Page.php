...
class Page {
  ...
  public function getHtmlScriptRootContainers(): string {
    $html = '';
    $explicitlyMountedScriptIds = array( 'wpml-notice-glossary', 'wpml-notice-improve-translations' );
    $this->scripts = array_filter(
      $this->scripts,
      function ( $script ) use ( $explicitlyMountedScriptIds ) {
        return ! in_array( $script->id(), $explicitlyMountedScriptIds, true );
      }
    );
