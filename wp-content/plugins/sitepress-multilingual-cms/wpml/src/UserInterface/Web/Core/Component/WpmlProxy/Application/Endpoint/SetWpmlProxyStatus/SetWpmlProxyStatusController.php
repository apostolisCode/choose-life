...
class SetWpmlProxyStatusController implements EndpointInterface {
  ...
  public function handle( $requestData = null ): array {
    try {
      ...
      $this->updateProxyStatus( $enabled, $this->isAutomatic( $requestData ) );
  ...
  private function isAutomatic( $requestData ): bool {
    return is_array( $requestData )
           && array_key_exists( 'automatic', $requestData )
           && true === $requestData['automatic'];
  }


  private function updateProxyStatus( bool $enabled, bool $automatic = false ) {
    if ( $automatic ) {
      $this->wpmlProxyService->applyAutomaticDecision( $enabled );
