...
class Repository implements RepositoryInterface {
  ...
  public function getFromItemsByToItem( Item $item ) {
    ...
    foreach ( self::TABLES_TO_BY_TYPE[ $item->getType() ] as $table ) {
      $itemId = $item->isOriginal() ? $item->getId() : $item->getIdOriginal();

      if ( self::TABLE_FROM_POST_TO_POST['name'] === $table['name'] ) {
        $itemIds = $wpdb->get_col(
          $wpdb->prepare(
            "SELECT id_from FROM {$wpdb->prefix}icl_links_post_to_post WHERE id_to = %d",
            $itemId
          )
        );
      } else {
        $itemIds = $wpdb->get_col(
          $wpdb->prepare(
            "SELECT id_from FROM {$wpdb->prefix}icl_links_post_to_term WHERE id_to = %d",
            $itemId
          )
        );
      }
  ...
  public function getToItemsByFromItem( Item $item ) {
    ...
    foreach ( self::TABLES_FROM_BY_TYPE[ $item->getType() ] as $table ) {
      if ( self::TABLE_FROM_POST_TO_POST['name'] === $table['name'] ) {
        $itemIds = $wpdb->get_col(
          $wpdb->prepare(
            "SELECT id_to FROM {$wpdb->prefix}icl_links_post_to_post WHERE id_from = %d",
            $item->getId()
          )
        );
      } else {
        $itemIds = $wpdb->get_col(
          $wpdb->prepare(
            "SELECT id_to FROM {$wpdb->prefix}icl_links_post_to_term WHERE id_from = %d",
            $item->getId()
          )
        );
      }
  ...
  public function addRelationship( Item $from, Item $to ) {
    ...
    if ( self::TABLE_FROM_POST_TO_POST['name'] === $table['name'] ) {
      $wpdb->query(
        $wpdb->prepare(
          "INSERT INTO {$wpdb->prefix}icl_links_post_to_post (id_from, id_to) VALUES (%d, %d) ON DUPLICATE KEY UPDATE id_to = %d",
          $from->getId(),
          $to->getId(),
          $to->getId()
        )
      ...
      $wpdb->query(
        $wpdb->prepare(
          "INSERT INTO {$wpdb->prefix}icl_links_post_to_term (id_from, id_to) VALUES (%d, %d) ON DUPLICATE KEY UPDATE id_to = %d",
          $from->getId(),
          $to->getId(),
          $to->getId()
        )
  ...
  public static function createDatabaseTables(): bool {
    ...
    $result = true;
    $creation = $wpdb->query(
      "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}icl_links_post_to_post (
    ...
    $creation = $wpdb->query(
      "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}icl_links_post_to_term (
        `id_from` bigint(20) unsigned NOT NULL,
        `id_to` bigint(20) unsigned NOT NULL,
        PRIMARY KEY (`id_from`,`id_to`),
        KEY `id_to` (`id_to`)
