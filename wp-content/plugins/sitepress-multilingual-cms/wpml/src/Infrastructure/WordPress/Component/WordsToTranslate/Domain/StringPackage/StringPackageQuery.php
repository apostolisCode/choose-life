...
class StringPackageQuery implements StringPackageQueryInterface {
  ...
  public function getById( $id ) {
    ...
    $ownerPostId = isset( $stringPackageRaw->post_id ) ? (int) $stringPackageRaw->post_id : 0;
    if ( $ownerPostId > 0 ) {
      $ownerPost = \get_post( $ownerPostId );
      if ( $ownerPost ) {
        $stringPackage->setDateModified(
          substr( $ownerPost->post_date, 0, 10 )
        );
      }
    }
