...
class TranslationQuery implements TranslationQueryInterface {
  ...
  public function getTranslationIdByJobId( int $jobId ) {
    $query = "SELECT `status`.`translation_id`
  ...
  public function getManyByElementIds(
    ...
    $type = $this->queryPrepare->prepare(
      ...
      $translationType->getElementTypePrefix() . '_%'
    );
    ...
    $query = $this->getBasicQueryWithMaxJobId() . $tridIn . $type;
  ...
  private function getBasicQuery(): string {
    return "
      ...
      ON `original`.`trid` = `translation`.`trid`
         AND `original`.`source_language_code` IS NULL
         AND `translation`.`source_language_code` IS NOT NULL
      INNER JOIN `{$this->queryPrepare->prefix()}icl_translation_status` AS `status`
  ...
  private function leftJoinWithMaxJobId(): string {
    return LatestTranslateJobSql::leftJoinLatestJob(
      $this->queryPrepare->prefix(),
      '`status`.`rid`'
    );
  }
