...
class JobPriorityService {
  ...
  public function __construct(
        ...
        ?TermDataQueryInterface $termDataQuery = null
    ) {
      $this->sorter          = $sorter;
      ...
      $this->termDataQuery   = $termDataQuery;
  }
  ...
  public function buildOrderingPayload(
    ...
    $items = array_merge( $items, $this->buildTermItems( $termIds ) );
  ...
  public function buildOrderingPayloadArray(
        array $postIds,
        array $stringIds = [],
        array $packageIds = [],
        array $termIds = []
    ): array {
      return $this->buildOrderingPayload( $postIds, $stringIds, $packageIds, $termIds )->toArray();
  }
  ...
  private function buildTermItems( array $termIds ): array {
    if ( empty( $termIds ) ) {
        return [];
    }

      $depths = $this->termDataQuery === null ? [] : $this->termDataQuery->getDepthMap( $termIds );
      $items  = [];

    foreach ( $termIds as $termId ) {
        $items[] = $this->itemBuilder->buildFromTerm( $termId, $depths[ $termId ] ?? 0 );
    }
