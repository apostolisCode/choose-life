...
use WPML\Core\Component\WordsToTranslate\Domain\Taxonomy\Provider as ProviderTaxonomy;
...
class WordsToTranslateService {
  ...
  public function getForIdAndType( $id, $type, $langs, $freshTranslation = false ) {
    switch ( $type ) {
      ...
      case ProviderTaxonomy::TYPE:
        return $this->getForTaxonomyTerm( $id, $langs, $freshTranslation );
      default:
  ...
  public function getForTaxonomyTerm( $idTermTaxonomy, $targetLangs, $freshTranslation = false ) {
    return $this->provider->getByIdAndTypeForLangs(
      $idTermTaxonomy,
      ProviderTaxonomy::TYPE,
      $targetLangs,
      $freshTranslation
    );
  }
  ...
  public function getForJob( $idJob, $freshTranslation = false, $withEvidence = false ) {
    return $this->providerJob->getById( $idJob, $freshTranslation, $withEvidence );
  }
