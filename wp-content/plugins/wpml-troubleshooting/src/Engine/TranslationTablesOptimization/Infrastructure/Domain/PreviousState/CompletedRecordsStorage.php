...
namespace WPML\Troubleshooting\Engine\TranslationTablesOptimization\Infrastructure\Domain\PreviousState;
...
class CompletedRecordsStorage implements CompletedRecordsStorageInterface {
  ...
  public function create() {
    try {
      if ( ! $this->databaseSchemaInfo->doesTableExist( self::TMP_TABLE_NAME ) ) {
        ...
        $this->wpdb->query(
          "CREATE TABLE {$wpdb->prefix}icl_translation_prevstate_processed (
  ...
  public function delete() {
    ...
    $wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}icl_translation_prevstate_processed" );
  }
  ...
  public function markAsCompleted( array $recordIds ) {
    ...
    $wpdb->query(
      $wpdb->prepare(
        "INSERT INTO {$wpdb->prefix}icl_translation_prevstate_processed (translation_id, processed)
