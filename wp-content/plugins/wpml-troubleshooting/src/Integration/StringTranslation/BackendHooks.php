...
class BackendHooks implements \IWPML_Backend_Action, \IWPML_DIC_Action {
	...
	public function displayButtons() {
		...
		if ( ! $this->domainsAndLanguagesRepo->get()->isEmpty() ) {
			$this->displayButton(
				...
				esc_attr__( 'Show custom MO Files Pre-generation dialog box', 'wpml-troubleshooting' ),
				false
			);
		}
		...
		$this->displayButton(
			...
			esc_attr__( 'Cleanup and optimize string tables', 'wpml-troubleshooting' ),
			esc_attr__( 'Cleanup and optimization completed!', 'wpml-troubleshooting' )
		);

		$this->displayLinkButton(
			admin_url( sprintf( 'admin.php?page=%s', WPML_ST_FOLDER . '/menu/string-translation.php' ) ) . '&amp;troubleshooting=1',
			esc_attr__( 'Check for string issues', 'wpml-troubleshooting' )
		);
	...
	public function loadJS( $hook ) {
		if ( WPML_PLUGIN_FOLDER . '/menu/troubleshooting.php' === $hook ) {
			wp_register_script(
				self::SCRIPT_HANDLE,
				WPML_TROUBLESHOOTING_URL . '/res/js/st-troubleshooting.js',
