...
use WPML\FP\Json;
use WPML\FP\Logic;
use WPML\Troubleshooting\Integration\ICLToATEMigration\ICLStatus;
use WPML\TM\API\TranslationServices;
use WPML\TM\TranslationProxy\Services\AuthorizationFactory;

class AuthenticateICL implements IHandler, Authorized {
	...
	public function run( Collection $data ) {
		return Either::of( $data->get( 'token' ) )
		             ->chain( Logic::ifElse(
			             Fns::identity(),
			             Either::of(),
			             Fns::always( Either::left( __( 'Token is not defined', 'wpml-troubleshooting' ) ) )
		             ) )
		             ->chain( Logic::ifElse(
			             [ $this->iclStatus, 'isActivatedAndAuthorized' ],
			             Fns::always( Either::left( __( 'ICanLocalize.com is already active and authorized', 'wpml-troubleshooting' ) ) ),
			             Either::of()
		             ) )
		             ->map( Logic::ifElse(
	...
	private function successResponse() {
		...
		return Fns::always( __( 'Service activated.', 'wpml-troubleshooting' ) );
	}

	private function errorResponse() {
		return function ( $errorDetails = '' ) {
			return [
				'message' => __( 'The authentication didn\'t work. Please make sure you entered your details correctly and try again.', 'wpml-troubleshooting' ),
